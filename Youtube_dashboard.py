...
median_agg = filtered_df[numeric_cols].median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    numeric_metrics = ['Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                      'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']
    
    df_agg_metrics = df_agg[['Video publish time'] + numeric_metrics]
    metric_date_6mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months =6)
    metric_date_12mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months =12)
    
    # Calculate medians only for numeric columns
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo][numeric_metrics].median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo][numeric_metrics].median()
    ...
    # Get only numeric columns for formatting
